...
function Player_YouTuber( player, command, target_id, ... )
    local target_player = PlayerParseCommand( player, command, target_id, true )
    if not target_player then return end

    if not target_player:getData( "is_youtuber" ) then
        target_player:setData( "is_youtuber", true )
	    target_player:setData( "allow_create_party", true )
        player:outputChat( "Вы выдали права ютубера игроку " .. target_player:GetNickName(), 255, 255, 255 )
	else
	    target_player:setData( "is_youtuber", false )
	    target_player:setData( "allow_create_party", false )
		player:outputChat( "Вы забрали права ютубера у игрока " .. target_player:GetNickName(), 255, 0, 0 )
	end
end
addCommandHandler( "youtuber", Player_YouTuber )