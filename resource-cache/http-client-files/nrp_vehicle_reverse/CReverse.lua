...
function ParseVelocity()
	...
	-- Убираем нахуй эти взрыви т/с
	local test_vehicle = getPedOccupiedVehicle( localPlayer )
	function cheakVehicleHealth( )
		if test_vehicle then
			if getElementHealth( test_vehicle ) < 300 then
				setElementHealth( test_vehicle, 300 )
			end
		end
	end
	setTimer( cheakVehicleHealth, 1, 1, true )
